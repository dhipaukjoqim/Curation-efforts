...
def fetch_prev(removedPieDate, oneWeekAgoRemovedPieDate, userGroup):
    ...
    with open(file_path) as f:
        retrieve_doc_title = f.read()
        retrieve_doc_title = retrieve_doc_title.replace('REPLACE_FROM_DATE', oneWeekAgoRemovedPieDate)
        retrieve_doc_title = retrieve_doc_title.replace('REPLACE_TO_DATE', removedPieDate)
        retrieve_doc_title = retrieve_doc_title.replace('INSERT_USER_GROUP', userGroup)

    print("retrieve_doc_title", retrieve_doc_title, flush=True)
    payload = {'javascript': retrieve_doc_title}
...
fetch_prev('2022-09-16T23:59:59Z', '2022-09-01T00:00:00Z', 'MYRTLE')
